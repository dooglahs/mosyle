...
LOCAL_BREW_VERSION=$( /usr/local/bin/brew --version | sed -n 1p | awk '{print $2}' )
ONLINE_BREW_VERSION=$( curl --silent "https://api.github.com/repos/Homebrew/brew/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/' )
if (( $(echo "${ONLINE_BREW_VERSION} ${LOCAL_BREW_VERSION}" | awk '{print ($1 == $2)}') )) || [[ ! -d /usr/local/Homebrew ]]; then
	echo "NO"
else
	echo "YES"
fi
